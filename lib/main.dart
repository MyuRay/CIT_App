...
void main() async {
  ...
  try {
    ...
    try {
      ...
      if (authStateResolved) {
        debugPrint('Firebase Auth認証状態復元完了: ${resolvedUser != null ? "ログイン済み" : "未ログイン"}');
      } else {
    ...
    if (currentUser != null) {
      ...
      debugPrint('既存ユーザーセッションなし');
      
      // SharedPreferencesからのバックアップ情報確認
      try {
        final wasLoggedIn = prefs.getBool('user_logged_in') ?? false;
        final backupUid = prefs.getString('user_uid') ?? '';
        final backupEmail = prefs.getString('user_email') ?? '';
        
        if (wasLoggedIn) {
          debugPrint('⚠️ SharedPreferencesには認証情報が残っています');
          debugPrint('⚠️ バックアップUID: $backupUid');
          debugPrint('⚠️ バックアップEmail: $backupEmail');
          debugPrint('⚠️ Firebase Authの復元に失敗している可能性があります');
        }
      } catch (prefsError) {
        debugPrint('⚠️ SharedPreferences確認エラー: $prefsError');
      }
    }
    
    debugPrint('Firebase初期化成功');
    debugPrint('初期化後のFirebase apps数: ${Firebase.apps.length}');
    debugPrint('Firebase app names: ${Firebase.apps.map((app) => app.name).toList()}');
...
class _CITAppState extends ConsumerState<CITApp> with WidgetsBindingObserver {
  ...
  void _checkAuthenticationOnResume() {
    try {
      // PersistentAuthProviderの強制チェックを実行
      final authNotifier = ref.read(persistentAuthProvider.notifier);
      authNotifier.forceCheck();
      
      // 最終アクセス時刻も更新
      authNotifier.updateLastAccess();
    } catch (e) {
